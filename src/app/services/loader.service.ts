...
import { BehaviorSubject, delayWhen, distinctUntilChanged, map, Observable, timer } from 'rxjs';
...
export class LoaderService {
  ...
  public hideLoader(counter?: number): void {
    const value = this.loading.getValue() - (counter && counter > 0 ? counter : 1);
    this.loading.next(value < 0 ? 0 : value);
  }

  public showLoader(counter?: number): void {
    this.loading.next(this.loading.getValue() + (counter && counter > 0 ? counter : 1));
  }
}
